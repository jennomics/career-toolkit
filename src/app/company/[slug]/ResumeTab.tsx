    ...
    <div className="space-y-4">
        ...
        {mode === "targeted" && (
          <div className="mb-4">
            ...
            {company.jobs.length === 0 ? (
              <p className="text-sm text-gray-400">No jobs for this company yet.</p>
            ) : (
              <select
                value={selectedJobId}
                onChange={(e) => setSelectedJobId(e.target.value)}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-purple-500 cursor-pointer"
              >
      ...
      {isGenerating && (
        <div className="text-center py-12">
          <div className="inline-block w-6 h-6 border-2 border-purple-600 border-t-transparent rounded-full animate-spin" role="status" aria-label="Generating resume" />
          <p className="text-gray-500 mt-3 text-sm">Generating company-scoped resume...</p>
        </div>
      )}
                ...
                <div className="space-y-5">
                  {resume.workExperience.map((role, i) => (
                    <div key={i} className="border-l-2 border-purple-200 pl-4">
                      ...
                      <p className="text-sm text-gray-600 mb-1.5">{role.company}{role.location ? ` - ${role.location}` : ""}</p>
